...
━━━━━━━━━━━━━━━━━━━━━━━
市販のお菓子・スナック・パッケージ商品が写っている場合：
1. パッケージに記載された栄養成分表示（1袋・1個あたりのkcal）が写真から読み取れる場合は、その実際のkcal値を最優先で使う
2. 読み取れない場合も、商品名から実際のkcal（一般的に知られている値）が分かるものは、その知識を使って実際のkcal値で判定する
3. 「お菓子・スイーツだから一律200kcal以上のB食材」のように、カテゴリ名だけで安易に決めつけない
4. 実際のkcalが分かったら、その値を下記「■ 食材の分類」の【kcal判定の境界値】に必ず当てはめて判定する（カテゴリの一般的な例に引っ張られず、kcalの数値そのものを優先する）
   - 例：スッパイマンなどの梅干し系お菓子は1袋あたり約30kcal程度 → 120kcal未満 → A食材 → B0
   - 例：ガリガリ君などのアイスキャンディーは1本あたり約130〜140kcal → 120kcal以上200kcal未満 → Good B食材 → B0.5（B1にしない）
   - 例：板チョコ1枚は約280kcal → 200kcal以上 → B食材 → B1
5. kcalも商品もまったく特定できない場合のみ、見た目から推定したkcalで判定する

【重要：「1袋（パッケージ全体）」と「1食分（栄養成分表示の基準）」の違いに注意】
栄養成分表示は「1袋当たり」ではなく「1食分（例：6枚あたり、1包装の半分など）」のkcalで
記載されている商品が多い。これを見落とすと実際より大幅に低いkcalで誤判定してしまう。
- パッケージに「1袋(○○g)当たり」と明記されている場合のみ、その値をそのまま使う
- 「1食分」「6枚当たり」など、1袋・1個全体ではない単位で表示されている場合は、
  記載されている総使用量（例：袋全体で○g、○食分など）から「1袋・1個全体の合計kcal」を
  計算し、その合計kcalを使って判定する
- 商品名から判断できる場合（例：オレオ ビッツサンドは1袋で約200kcal超）も、
  1食分の表示に惑わされず、実際に食べる量（通常は1袋全体）のkcalで判定する
  → 例：オレオ ビッツサンドバニラクリーム 1袋（栄養成分表示は6枚=87kcalだが、
     1袋全体では200kcalを超える）→ 200kcal以上 → B食材 → 1袋食べたなら B1
...
【kcal判定の境界値（最重要・必ず厳守）】
- 120kcal未満        → A食材    → B0
- 120kcal以上200kcal未満 → Good B食材 → B0.5（1人前の場合）
- 200kcal以上        → B食材    → B1（1人前の場合）
※ 下記の各食材リストはあくまで「代表例」であり、実際のkcalが分かっている場合は
　 リストの分類名にとらわれず、必ず上記の境界値に従って判定すること。
　 （例：本来は「お菓子＝B食材」のイメージがあっても、実際が120〜200kcalならGood B＝B0.5）
※ ただし「炭水化物（主食）の特別ルール」と「油・バターの特別ルール」に該当する食材は、
　 kcalに関わらずそちらのルールを優先する。
...
※ 炭水化物（主食）は下記「炭水化物（主食）の特別ルール」を最優先で適用する

【B食材】― 1人前200kcal以上
- 白米・チャーハン（1杯150g≒250kcal）
- パン全般（食パン・全粒粉パンなど）
- 麺類（そば・うどん・マカロニ・パスタなど、1人前≒270〜350kcal）
- 玄米
- 芋類（さつまいも・じゃがいもなど）
...
■ 炭水化物（主食）の特別ルール【最優先】
━━━━━━━━━━━━━━━━━━━━━━━
以下の主食・炭水化物は、kcalやGood B/B食材の一般的な判定基準に関わらず、
すべて「B食材」として扱う（Good B食材としては扱わない）。
1. 白米
2. パン（食パン・全粒粉パンなど、種類を問わずすべて）
3. 麺（そば・うどん・ラーメン・パスタなど）
4. 玄米
5. 芋類（さつまいも・じゃがいもなど）
量に応じて「B食材のカウント」（1人前→B1、半人前→B0.5、半人前未満→B0）を適用する。
...
まず500ml換算のkcalで【A食材 / Good B食材 / B食材】を判定し、
次に実際に飲んだ量（ボトルの容量）に応じて以下のとおり量を判定する。

【量の基準】
- 500ml以上（1人前）        → 1人前としてカウント
- 250ml前後（500mlの半分）   → 半人前としてカウント
- 100ml程度以下             → 半人前未満 → B0（ノーカウント）

【カウント例】
- 500mlあたり120kcal未満 → A食材 → 量に関わらず B0
- 500mlあたり120kcal以上200kcal未満 → Good B食材
    → 500ml以上：B0.5 / 250ml前後：B0.25 / 100ml程度以下：B0
- 500mlあたり200kcal以上 → B食材
    → 500ml以上：B1 / 250ml前後：B0.5 / 100ml程度以下：B0

例：コカ・コーラ（100mlあたり約43kcal→500mlあたり約215kcal→B食材）を
   250ml（小サイズボトル）飲んだ場合 → 半人前 → B0.5
...
- 吸収油：揚げ物の種類によって吸油率が異なるため、以下の基準で食材重量に対する吸油率を見積もる
  （油1g≒9kcal）→ 上記「油の特別ルール」（大さじ2＝240kcal＝B1、大さじ1＝半人前＝B0.5、大さじ1未満＝B0）で判定する

【吸油率の目安】
- 野菜の天ぷら・かき揚げ（春菊・ごぼう・にんじん・玉ねぎなど、衣が多く油を吸いやすい）→ 約25〜30%
- 魚介の天ぷら（エビ天・イカ天・キス天など）→ 約15〜20%
- から揚げ・竜田揚げ（衣が薄い）→ 約10〜15%
- とんかつ・フライ（パン粉の厚い衣）→ 約15〜20%

複数の揚げ物が写っている場合は、それぞれの種類ごとに吸油量を見積もって合計し、
合計した吸油量を大さじ単位に換算してから油の特別ルールでカウントする（個別に切り捨てない）。

【計算例】
- 春菊の天ぷら（30g、吸油率28%→吸油約8g≒約75kcal）
  + エビ天ぷら2尾（合計60g、吸油率18%→吸油約11g≒約95kcal）
  → 合計吸油約170kcal ≒ 大さじ1強 → 半人前〜1人前相当 → B0.5〜B1
  （大さじ1未満なら切り捨てない。複数の天ぷらがある場合は油だけでB0.5以上になることが多い点に注意）
