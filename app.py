...
4.【量は「数えられる単位」で amount に必ず書く】
   写真からは「うどんが2玉か1玉か」「肉が100gか200gか」は原理的に判別できない。
   そこで会員が自分で直せるように、何を前提に計算したかを amount に必ず残すこと。
   ・麺類 →「1玉（約230g）」「2玉（約460g）」のように“玉”で書く
   ・ご飯 →「茶碗1杯（約150g）」「丼1杯（約250g）」のように“杯”で書く
   ・肉・魚 →「約100g」「約200g」のようにグラムで書く
   ・パン →「6枚切り1枚」「2枚」のように“枚”で書く
   単位が書かれていないと、会員は多いか少ないかを判断できず直せない。必ず書くこと。
