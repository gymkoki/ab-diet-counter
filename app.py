...
※ 肉・魚でグッドBに該当するものは下記「タンパク質食材の分類ルール」で判定する
...
- マグロ赤身：約115kcal
- タラ・ヒラメ・カレイ・アジ（赤身系）などの白身・低脂質魚：約70〜100kcal
- えび・いか・たこ・ホタテ・貝類：約80〜100kcal
- 木綿豆腐・絹豆腐：約56〜70kcal

【グッドB食材：100gあたり120〜199kcal】― 実際のkcalで0〜1カウント
- 皮付き鶏もも肉：約160kcal
- 豚もも肉（赤身）：約128kcal
- 牛もも肉（赤身）：約140kcal
...
【B食材：100gあたり200kcal以上】― 実際のkcalで0〜2カウント
- 鮭・サーモン（生・焼き・ムニエルなど）：約200〜220kcal　※グッドBではなくB食材
- ブリ（生・焼き）：約257kcal
- サバ（生・焼き）：約247kcal
- サンマ（生・焼き）：約318kcal
- ウナギ（蒲焼き）：約255kcal
...
- チーズ（1人前50g以上で200kcal超）

【カウント例：鮭（B食材、1人前100g≒210kcal）】
- 約30g（約63kcal）→ 120kcal以下 → 0カウント
- 約80g（約168kcal）→ 121〜199kcal → B0.5カウント　← 半人前
- 約100g（約210kcal）→ 200kcal以上 → B1カウント　← 1人前
- 約150g以上（約315kcal以上）→ B1カウント（普通の大盛り）
...
料理に含まれるB食材・グッドB食材をそれぞれ独立して分解し、個別にカウントする。

【原則2：B1は「B食材を1人前食べたとき」のみ付く】
- B食材を1人前（200kcal以上）食べた → B1
- B食材を半人前（121〜199kcal）食べた → B0.5
- B食材を半人前未満（120kcal以下）食べた → B0
...
- 実際のカロリーが120kcal未満（少量・トッピング程度）→ 0カウント
...
【B食材のカウント】
写真に写っている実際の量のカロリーを推定して判定：
- 実際のカロリーが120kcal以下（少量・具材・トッピング程度）→ 0カウント
- 実際のカロリーが121〜199kcal（半人前相当）→ 0.5カウント
- 実際のカロリーが200kcal以上（1人前）→ 1カウント
- 実際のカロリーが200〜399kcal程度（大盛り）→ 1カウント（普通の大盛りはB2にならない）
- 実際のカロリーが400kcal以上（超大盛り・2人前以上）→ 2カウント

「大盛り」は必ずしもB2ではない。B2になるのは「2人前に近い超大盛り」の場合のみ。

  例）白米（1人前150g≒250kcal）
  ・少量（30g、約50kcal）→ 120kcal以下 → 0カウント
  ・半膳（75g、約125kcal）→ 121〜199kcal → 0.5カウント
  ・1杯（150g、約250kcal）→ 200kcal以上 → 1カウント
  ・普通の大盛り（200g、約333kcal）→ 1カウント（B2にはならない）
  ・超大盛り（300g以上、約500kcal以上）→ 2カウント

  例）鮭・サーモン（B食材、1人前100g≒210kcal）
  ・少量（約30g、約63kcal）→ 120kcal以下 → 0カウント
  ・約80g（約168kcal）→ 121〜199kcal → B0.5カウント（写真で「1切れ・小〜中サイズ」程度）
  ・1人前（約100g、約210kcal）→ 200kcal以上 → B1カウント

  例）ブリ・サバ（B食材、1人前100g≒247〜257kcal）
  ・少量（約30g、約75kcal）→ 120kcal以下 → 0カウント
  ・約60g（約148〜154kcal）→ 121〜199kcal → B0.5カウント
  ・1人前（約100g、約247〜257kcal）→ 200kcal以上 → B1カウント

  例）油・ドレッシング（1人前＝大さじ2≒200kcal）
  ・小さじ1（約40kcal）→ 120kcal以下 → 0カウント
  ・大さじ1（約110kcal）→ 120kcal以下 → 0カウント
  ・大さじ1〜1.5程度（約121〜165kcal）→ 121〜199kcal → 0.5カウント
  ・大さじ2以上（約200kcal以上）→ 1カウント
...
→ タンパク質食材の分類ルールに従い、実際のkcalでSTEP2のしきい値を適用する
  ...
  - 鶏もも肉（皮付き）→ グッドB（100gあたり約160kcal）→ 実際のkcalで判定
...
→ 揚げ物は食材重量の約20%の油を吸収する。その吸収油のカロリーで以下のルールを適用する：
  - 吸収油が120kcal未満 → 0カウント
  - 吸収油が120kcal以上200kcal未満 → グッドB扱い → 0.5カウント
  - 吸収油が200kcal以上 → B食材扱い → 1カウント

【吸収油カロリーの目安（油1g≒9kcal）】
  - 薄衣の唐揚げ 100g → 吸油約15g（135kcal）→ 0.5カウント
  - 厚衣のフライ・フライドチキン 100g → 吸油約20〜25g（180〜225kcal）→ 0.5〜1カウント
  - 大量の揚げ物（200g超）→ 吸油が200kcal超えることが多い → 1カウント

【具体例】
- 鶏もも肉（皮付き）唐揚げ 1人前（100g）
  1. 鶏もも肉（皮付き）= B食材、1人前 → B1カウント
  2. 衣（小麦粉・卵）= 少量 → 0カウント
  3. 吸油 約15g（135kcal）→ グッドB → 0.5カウント
  ✅ 合計：B1.5カウント
...
- フライドチキン（皮付き・厚衣）1人前（100g）
  1. 鶏もも肉（皮付き）= B食材、1人前 → B1カウント
  2. 衣 = 少量 → 0カウント
  3. 吸油 約22g（200kcal）→ B食材 → B1カウント
  ✅ 合計：B2カウント

- 野菜の天ぷら 2〜3個（合計80g）
  1. 野菜 = A食材 → 0カウント
  2. 衣 = 少量 → 0カウント
  3. 吸油 約16g（144kcal）→ グッドB → 0.5カウント
  ✅ 合計：B0.5カウント

- 野菜の素揚げ（少量・40g程度）
  1. 野菜 = A食材 → 0カウント
  2. 吸油 約8g（72kcal）→ 120kcal未満 → 0カウント
  ✅ 合計：B0カウント
