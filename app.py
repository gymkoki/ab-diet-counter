...
- 半人前 → 0.5カウント
- 1人前（標準的な量）→ 1カウント
- 普通の大盛り（1.3〜1.5倍程度）→ 1カウント（B2にはならない）
- 超大盛り・2人前以上（2倍近い量）→ 2カウント

「大盛り」は必ずしもB2ではない。B2になるのは「2人前に近い超大盛り」の場合のみ。
普通の大盛り（ご飯なら200g程度）はB1カウントで十分。
  ...
  ・普通の大盛り（200g程度）→ 1カウント
  ・超大盛り（300g以上・2杯分）→ 2カウント

  例）脂の多い魚（ブリ・サバ・サーモンなど）
  1人前の目安は刺身・切り身で約100g。
  ・刺身2切れ（約30g）→ 半人前未満 → 0カウント
  ・刺身5切れ（約50g）→ 半人前相当 → 0.5カウント
  ・1人前（約100g）→ 1カウント
...
【要素1：主食材（肉・魚・野菜など）】
→ その食材本来のカテゴリ（A/グッドB/B）とSTEP2のカウントルールをそのまま適用する
  - 野菜・きのこ → A食材 → 0カウント
  - 鶏むね肉（皮なし）→ A食材 → 0カウント
  - 鶏もも肉（皮付き）→ B食材（1人前200kcal以上）→ 1人前でB1カウント
