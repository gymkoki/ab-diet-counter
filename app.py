...
【手順2・最優先】商品名やパッケージが写っていて、正確なカロリーに自信がないときは、
   **web_search ツールで商品名＋「カロリー」を検索し、実際の栄養成分（kcal）を確認してから判定する**。
   （例の検索語：「セブンイレブン チョコバナナアイス カロリー」「オレオ ビッツサンド 1袋 カロリー」）
   - 検索するのは「市販のパッケージ商品・コンビニ商品・チェーン店のメニュー」で、カロリーが不確かなときだけ。
     手料理・家庭料理・一般的な食材（ご飯・肉・野菜など）では検索しない（時間と費用の無駄になる）。
   - 検索は1回の解析で最大2回まで。見つからなければ手順3の推定に進む。
...
def analyze():
    ...
    try:
        result = create_and_parse(
            ...
            # 市販のパッケージ商品（コンビニ菓子など）が写っているときだけ、AIが自分で
            # 商品名＋カロリーをWeb検索して実測値を確認できるようにする。
            # 検索して良い場面はANALYSIS_PROMPTの【手順2】で限定しているため、
            # 手料理の写真では検索が走らず、費用も時間も増えない。
...
━━━━━━━━━━━━━━━━━━━━━━━"""
    ...
    try:
        result = create_and_parse(
            ...
            thinking=THINKING_OFF,   # 思考オフ（従来と同じ挙動・コスト増を防ぐ）
            # 「これは○○という商品です」と訂正されたときに、AIが実測カロリーを
            # Web検索で確認できるようにする（本解析と同じ条件で限定的に使う）。
            tools=[WEB_SEARCH_TOOL],
            max_tokens=16000,
            system=[
                {
